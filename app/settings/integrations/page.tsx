...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    const checkIntegrationStatus = async () => {
      try {
        ...
        if (data) {
          if (data.mt4_api_key) {
            const [broker] = data.mt4_api_key.split(':')
            setConnected(broker)
          }
  ...
  const handleBrokerConnect = async () => {
    ...
    try {
      console.log('[v0] Saving MetaTrader credentials...')
      ...
      console.log('[v0] MetaTrader credentials saved successfully')
      setConnected(selectedBroker)
      setCredentials({ login: '', password: '' })
      setSelectedBroker(null)
      setSuccess(`${BROKERS.find(b => b.id === selectedBroker)?.name} connected successfully!`)
      setTimeout(() => setSuccess(''), 3000)
    } catch (err) {
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-emerald-300">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-emerald-200/70">Your trades will sync automatically</p>
                  </div>
                </div>
                <Button
