...
function shieldColor(pct: number, isEn: boolean) {
  if (pct > 75) return { stroke: "#10b981", text: "text-emerald-400", label: isEn ? "Safe Zone" : "Bezpečná zóna" }
  if (pct > 50) return { stroke: "#3b82f6", text: "text-blue-400",    label: isEn ? "Caution" : "Pozor" }
  if (pct > 25) return { stroke: "#f59e0b", text: "text-amber-400",   label: isEn ? "Warning!" : "Varování!" }
  return           { stroke: "#f43f5e", text: "text-rose-400",         label: isEn ? "CRITICAL" : "KRITICKÉ" }
}
...
export default function TradingGoalsPage() {
  ...
  const generateGoalsWithAI = async () => {
    try {
      ...
      if (data.success && data.goals) {
        ...
        toast({ title: isEn ? "Done!" : "Hotovo!", description: isEn ? "Process goals generated by AI" : "Procesní cíle vygenerovány AI" })
      } else {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : isEn ? "Failed to generate goals." : "Nepodařilo se vygenerovat cíle.",
        variant: "destructive",
      })
    } finally {
      setGenerating(false)
  ...
  const shield = shieldColor(stats.drawdownShield, isEn)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-slate-400 text-sm max-w-lg">
              {isEn
                ? "No financial targets. Your only goal is to follow the process — profit is a byproduct of iron discipline."
          ...
          >
            {generating ? (
              <><Loader className="w-4 h-4 mr-2 animate-spin" />{isEn ? "Generating..." : "Generuji..."}</>
            ) : (
              <><Sparkles className="w-4 h-4 mr-2" />{isEn ? "Generate with AI" : "Vygenerovat s AI"}</>
            )}
          </Button>
        </div>
        ...
        {isShowingDemo && (
          <div className="inline-flex mb-8 mt-3 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-medium">
            {isEn ? "Demo data — this is what it looks like with your own results" : "Ukázková data — takhle to bude vypadat s tvými vlastními výsledky"}
          </div>
        )}
          ...
          <Card className="bg-gradient-to-br from-emerald-900/30 to-slate-900/60 border-emerald-500/20">
            <CardContent className="p-6 flex flex-col items-center gap-4">
              ...
              <CircularGauge value={stats.dailyProcessScore} color="#10b981">
                ...
                <span className="text-[10px] text-emerald-400 font-semibold">{isEn ? "TODAY" : "DNES"}</span>
              </CircularGauge>
              <div className="text-center">
                <p className="text-sm font-semibold text-white">
                  {stats.dailyProcessScore === 100
                    ? (isEn ? "Perfect day! 🌟" : "Perfektní den! 🌟")
                    : (isEn ? `${Math.round(stats.dailyProcessScore / 100 * 6)}/6 habits` : `${Math.round(stats.dailyProcessScore / 100 * 6)}/6 návyků`)}
                </p>
                <p className="text-[11px] text-slate-400 mt-0.5">{isEn ? "Even on a losing day it can hit 100%" : "I při ztrátě může svítit 100 %"}</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className={cn("text-sm font-semibold", stats.drawdownShield <= 20 ? "text-rose-300" : "text-white")}>
                  ${(stats.drawdownMax - stats.drawdownUsed).toLocaleString()} {isEn ? "remaining" : "zbývá"}
                </p>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {isEn ? `of $${stats.drawdownMax.toLocaleString()} weekly limit` : `z $${stats.drawdownMax.toLocaleString()} týdenního limitu`}
                </p>
                {stats.drawdownShield <= 20 && (
                  <p className="text-[10px] text-rose-300 mt-1 font-bold animate-pulse">🚨 {isEn ? "CLOSE THE PLATFORM NOW!" : "ZAVŘI PLATFORMU HNED!"}</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6 flex flex-col items-center gap-4">
              ...
              <CircularGauge value={stats.monthlyHabitScore} color="#a855f7">
                ...
                <span className="text-[10px] text-purple-400 font-semibold">
                  {new Date().toLocaleString(isEn ? "en-US" : "cs-CZ", { month: "long" }).toUpperCase()}
                </span>
              </CircularGauge>
              <div className="text-center">
                <p className="text-sm font-semibold text-white">
                  {stats.monthlyDaysClean}/{stats.monthlyDaysTotal} {isEn ? "clean days" : "čistých dní"}
                </p>
                <p className="text-[11px] text-slate-400 mt-0.5">{isEn ? "Days without plan violations" : "Dny bez porušení plánu"}</p>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[11px] text-slate-600 mt-0.5">{isEn ? "Discipline = green. See the pattern?" : "Disciplína = zelená. Vidíš ten vzorec?"}</p>
              </div>
              <div className="flex items-center gap-3 text-[10px] text-slate-500">
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-emerald-500 inline-block" />{isEn ? "Clean day" : "Čistý den"}</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-rose-500 inline-block" />{isEn ? "Incident" : "Incident"}</span>
              </div>
            </div>
        ...
        <Card className="bg-slate-900/50 border-slate-800 mb-6">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <p className="text-sm font-bold text-white">{isEn ? "Discipline → Win Rate Correlation" : "Disciplína → Win Rate korelace"}</p>
                <p className="text-[11px] text-slate-400">{isEn ? "Last 29 trading days of your data" : "Posledních 29 obchodních dní tvých dat"}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {/* With routine */}
              <div className="p-4 rounded-xl bg-emerald-900/20 border border-emerald-500/20">
                <p className="text-[10px] font-semibold text-emerald-400 uppercase tracking-wide mb-3">{isEn ? "Days with clean discipline" : "Dny s čistou disciplínou"}</p>
                ...
                <div className="mt-3 pt-3 border-t border-emerald-500/20 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500">{IMPACT_DATA.withRoutine.days} {isEn ? "days" : "dní"}</span>
                </div>
              </div>
              {/* Without routine */}
              <div className="p-4 rounded-xl bg-rose-900/20 border border-rose-500/20">
                <p className="text-[10px] font-semibold text-rose-400 uppercase tracking-wide mb-3">{isEn ? "Days with incident" : "Dny s incidentem"}</p>
                ...
                <div className="mt-3 pt-3 border-t border-rose-500/20 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500">{IMPACT_DATA.withoutRoutine.days} {isEn ? "days" : "dní"}</span>
                </div>
              </div>
            </div>
            <div className="mt-4 p-3 rounded-lg bg-slate-800/40 border border-slate-700/50">
              <p className="text-[11px] text-slate-400 leading-relaxed">
                <span className="text-emerald-400 font-bold">{isEn ? "+47pp win rate difference" : "+47pp rozdíl ve win rate"}</span>{" "}
                {isEn ? "between clean and incident days. Discipline isn't just a feeling — it's a numeric edge." : "mezi čistými a incidentovými dny. Disciplína není jen pocit — je to edge v číslech."}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900/50 border-slate-800 mb-6">
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{isEn ? "Limits on bad habits — crossing them = loss of discipline, not just money" : "Limity na nešvary — překročit = ztráta disciplíny, ne jenom peněz"}</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {stats.badHabits.map((h) => {
                ...
                const statusText = isHardStop
                  ? h.used === 0 ? "✅ Perfect" : (isEn ? `🚨 ${h.used}× VIOLATED` : `🚨 ${h.used}× PORUŠENO`)
                  : h.used === h.limit ? (isEn ? "🔴 Limit reached" : "🔴 Limit vyčerpán") : (isEn ? `${h.limit - h.used} remaining` : `${h.limit - h.used} zbývá`)
            ...
            <div className="mt-4 p-3 rounded-lg bg-slate-800/40 border border-slate-700/50">
              <p className="text-[11px] text-slate-400 leading-relaxed">
                <span className="text-slate-300 font-semibold">{isEn ? "Why this works: " : "Proč tohle funguje: "}</span>
                {isEn
                  ? "Regular apps tell you to make $500 per day — which forces you to gamble. Here your only goal is to keep Bad Habits Quota at zero. Profit then becomes a natural result of iron discipline."
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{isEn ? "Hard rules you must never break — no exceptions, no negotiation" : "Pevná pravidla, která nesmíš nikdy porušit — bez výjimky, bez diskuze"}</p>
              </div>
            </div>
          </CardHeader>
            ...
            {weeklyGoals.length > 0 && (
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  <Zap className="w-4 h-4 text-amber-400" />
                  <h2 className="text-lg font-bold text-white">{isEn ? "Weekly process goal" : "Týdenní procesní cíl"}</h2>
                  <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30 border text-xs">{isEn ? "This week" : "Tento týden"}</Badge>
                </div>
                  ...
                  {weeklyGoals.map((goal) => (
                    <Card key={goal.id} className="bg-slate-900 border-slate-700 hover:border-amber-500/40 transition-all">
                      <CardContent className="pt-6 space-y-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex flex-col items-end gap-2 shrink-0">
                            ...
                            <span className="text-xs text-slate-500">
                              {new Date(goal.startDate).toLocaleDateString(isEn ? "en-US" : "cs-CZ")} –{" "}
                              {new Date(goal.endDate).toLocaleDateString(isEn ? "en-US" : "cs-CZ")}
                            </span>
                          </div>
                        </div>
                        {goal.why && (
                        ...
                        <div>
                          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">{isEn ? "How to achieve it" : "Jak to dosáhnout"}</p>
            ...
            {monthlyGoals.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-lg font-bold text-white">{isEn ? "Monthly process goal" : "Měsíční procesní cíl"}</h2>
                  <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30 border text-xs">{isEn ? "This month" : "Tento měsíc"}</Badge>
                </div>
                <div className="grid gap-4">
        ...
        {displayGoals.length === 0 && (
          <div className="p-12 bg-slate-800/30 border border-dashed border-slate-700 rounded-xl text-center text-slate-400">
            ...
            <p className="font-medium text-slate-300 mb-1">{isEn ? "No goals yet" : "Žádné cíle zatím"}</p>
            <p className="text-sm">{isEn ? 'Click "Generate with AI" and AI will analyze your trading style' : 'Klikni na "Vygenerovat s AI" a AI analyzuje tvůj obchodní styl'}</p>
          </div>
        )}
