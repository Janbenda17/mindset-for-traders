...
async function handleCheckoutCompleted(
  ...
  if (error) {
      ...
      // Fire Purchase event to Meta CAPI - server-to-server, so it doesn't
      // depend on the buyer's browser at all (no ad blocker can stop this).
      if (isPremium) {
              try {
                        const amount = typeof session.amount_total === "number" ? session.amount_total / 100 : undefined
                        const currency = session.currency ? session.currency.toUpperCase() : "CZK"
                        await sendMetaConversionEvent({
                                    eventName: "Purchase",
                                    eventId: `purchase_${session.id}`,
                                    eventSourceUrl: "https://www.mindtrader.cz/upgrade",
                                    email,
                                    value: amount,
                                    currency,
                        })
                        ...
                        console.error("[WEBHOOK] Meta Purchase CAPI event failed:", metaErr)
              }
      }
  } else {
